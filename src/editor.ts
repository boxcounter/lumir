 ...
 *  调用方按「只能停靠左缘」处理。 */
function cellContentRange(state: EditorState, cell: { from: number; to: number }): { from: number; to: number } {
 ...
 *  落点自身仍走 lineBoundaryTarget 的可停靠回退（下界为归属 cell 内容的左缘），因此不会跨回
 *  上一个 cell。
 *
 *  导出供单测直接驱动：这是纯函数（只读 EditorState，不碰 view 与坐标测量），⌃E 的 cell 归属
 *  判定全在这里——落在 view 侧的只有「落点是否可停靠」的回退。 */
