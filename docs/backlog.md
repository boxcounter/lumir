...
- **`click` 动作不支持修饰键**（M144，medium）：`lib/execute.mjs` 的 click 只有 `target` / `count`，没有
  ...
  本批次改用视觉场景覆盖该路径（`tests/visual/scenes/render-link.spec.ts`：stub 记录
...
- **`file` 断言只吃字面路径，日志内容无法机验**（M144，medium）：诊断日志文件名是 UTC 日期
  （`env:logs/<UTC 日期>.jsonl`），`glob` 按路径正则计数、不读内容，写死日期会在跨日运行时读到旧文件而
  **静默假绿**（验收环境的 `env/` 目录跨天复用）。后果：真机侧无法机验「opener 真的被调用」——M144 的
  `link_open` 事件只能人工 `grep`（见 `scripts/acceptance/scenarios/12-links.md` 正文）。修法：给 `file`
  断言加 glob 化路径（如 `env:logs/*.jsonl` 取最新一份再断言内容），或把诊断日志落成固定文件名。
