...
6. **性能合同**（四个数字的测量方法学——端点、工具链、采样口径、fixture 规格——委托给 M0 产出的性能测量方法学 spec 定义，见 ADR 0004 M0，本 ADR 只锁定数字与阈值）：
   - 冷启动 <300ms
   - keypress-to-paint <16ms（一帧）
   - 打开 1MB Markdown 文件 <100ms
   - 常驻内存 <200MB

   CI 门禁口径（2026-09-05 一次性校准后）：冷启动与 keypress-to-paint 的 CI 门禁用滚动基线相对回归，其绝对合同值保留、在标准化环境（裁决者本机）按本条既有口径裁决；打开 1MB Markdown 文件与常驻内存保持 CI 绝对阈值门禁（回归即拒合）。相对回归的具体容忍线与统计口径不钉死在本 ADR——本 ADR 只锁合同数字，相对回归的比较统计量、滚动窗口与容忍线以 docs/specs/perf-measurement.md「相对回归门禁」一节为准（该 spec 是 M0 委托产物，见上文方法学委托）。2026-09-05 门禁口径修正：相对回归门禁比较值由 p95 改为 median、容忍线由 20% 放宽至 40%，属门禁实现的统计口径修订而非合同校准——合同数字与一次性校准额度已用尽的状态均不受影响。依据是 M37 误报调查的证据链（17 次 CI run 回放，完整记录见 spec）：基线窗口内 4 次绿 run 的 cold-start p95 自身散布达 43%、已超 20% 容忍线；逐字节相同代码的背靠背 run cold-start p95 284.4ms vs 808.9ms（2.8 倍纯 runner 方差）；keypress-to-paint 9 次 run 8 次红而同指标 median 未越线；17 次 run 的 runner 镜像完全相同，排除镜像漂移。
...
- 性能合同的四个数字是首批未经实测的目标值，M0 末四项指标全量实测（ADR 0004 M0 出口）后允许一次性校准（校准需修订本 ADR）。**该一次性校准额度已于 2026-09-05 使用**（即第 6 条的 CI 门禁口径修订）：依据是 CI（macos-15 runner）两次全量实测的噪声水平——冷启动 p95 284ms vs 809ms、keypress-to-paint p95 70ms vs 255ms，同一代码无变更下噪声达 3-4 倍，CI 绝对阈值对这两项重指标不可执行；而打开 1MB 文件 p95 约 2.6ms 与常驻内存约 110MB 两次几乎一致，噪声远未淹没信号，保持 CI 绝对阈值门禁。

## Revisit 条件

- 性能门禁反复失败且瓶颈确证为 webview 渲染层 → 重评 GPUI 路线。
- 裁决者对"极致快"的定义升级（价值判断变化）→ 全线重估。
- M0 末四项指标首次全量实测表明四个性能数字系统性不可达 → 一次性校准并修订本 ADR（仅允许这一次）。**该额度已于 2026-09-05 使用**（校准为"重指标 CI 相对回归 + 本机绝对值裁决，轻指标保持 CI 绝对阈值"，见第 6 条与 Consequences），此后不再可用。
