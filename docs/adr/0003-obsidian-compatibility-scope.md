...
4. **兼容性本体 = 冻结的 spec 文档 + fixture 测试集**。fixture 为按冻结 spec 人工合成的 fixture 集，覆盖 spec 的全部语法形态（2026-09-05 裁决：弃用原方案"取自作者真实 vault 的子集"——仓库已 public，真实 vault 子集不能进公开仓库）；该 spec 同时作为 AI agent 的开发指令。
...
- 合成 fixture 失去真实 vault 对 Obsidian 方言长尾行为的覆盖；dogfood 期发现长尾问题时由作者人工补充用例（不进仓库历史，公开仓库只含合成 fixture）。
