...
1. **13 周里程碑**：
   ...
   | 阶段 | 周 | 内容 | 出口标准 |
   |---|---|---|---|
   | M0 harness | 1-2 | CI 性能门禁、视觉回归、ADR/spec 流程、repo 骨架 | 空壳 app 的冷启动已在 CI 里被测量 |
...
4. **OpenSpec 引入方式**：M0 第 2 周引入，以 wikilink 功能（有冻结 spec、有 fixture，最适合验证）跑"提案→评审→实现→归档"全循环，两周后评估：评审制品是否真的让 Alex 不看代码也能裁决。不达标则退化为 `docs/proposals/` + 模板的自建约定，不留恋。
...
- OpenSpec 两周验证不达标 → 退化自建约定，修订本 ADR 第 4、5 条。
