...
4. **OpenSpec 引入方式**：M0 第 2 周引入，以性能测量方法学 spec 为验证对象跑第一个"提案→评审→实现→归档"全循环——它是 M0 自身产物、验收口径明确、第 2 周必然存在（wikilink 在 M1 才开发，M0 无可实现对象，不适合作为首个验证对象）。wikilink 的 OpenSpec 循环推迟到 M1：其前置任务"冻结 wikilink spec + 从作者真实 vault 提取 fixture"（ADR 0003 第 4 条）安排在 M1 第 1 周（第 3 周）完成，随后 wikilink 跳转开发跑完整循环。**评估窗口**（2026-09-05 裁决者裁决）：自首个全循环跑通起算，跑满 ≥3 个全循环或满两周，先到为准；评估内容是评审制品是否真的让 Alex 不看代码也能裁决。窗口口径由纯 wall-clock 改为循环数优先，理由：AI-only 开发速度下 wall-clock 不再是工作量的有效代理，评估的真正单位是循环数——两周内可能只跑了一个循环（样本不足），也可能已跑完五个（证据早已充分）。不达标则退化为 `docs/proposals/` + 模板的自建约定，不留恋。
