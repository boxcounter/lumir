# Wikilink 链接语义 spec（冻结 v1.1）

- 状态: frozen v1.1
- 日期: 2026-09-05
- 修订: v1.1（2026-09-05）——裁决点 G/H 按推荐值定稿；裁决点 I 按 Alex 改判定稿：一键创建位置由 vault 根改为当前文件所在目录（§4.4）
...
2. **短路径匹配**：path 段不含 `/` 时，按两个子步构造候选集，命中即停：
   ...
   候选集为空 → `unresolved`（未创建链接，§4.3）；恰一个候选 → `resolved`；多个候选 → `ambiguous`，按裁决点 G 的规则选出 `chosen`。
3. path 段含 `/` 但第 1 步未命中时，按"路径后缀匹配"再走一次候选集逻辑（vault 内路径以 `/<path>.md` 或 `/<path>` 结尾的文件），候选集构造与判定规则同第 2 步。

**裁决点 G——短路径歧义的选取规则**（2026-09-05 Alex 裁决：按推荐值定稿）：

- 候选按相对路径的**段数最少**优先；并列时取路径**字典序第一**；
- 选出的 `chosen` 正常参与跳转，但链接 MUST 带歧义标识（§4.2），让用户知道存在其他同名笔记；
- 备选方案（已否决）：仿 Obsidian"最短路径优先"但不给歧义标识——静默猜测在知识库里是错误放大器。
...
- 段文本去首尾空白后比较；
- **裁决点 H——标题锚点匹配的大小写**（2026-09-05 Alex 裁决：按推荐值定稿）：**大小写敏感**精确匹配。理由：标题文本是作者写的自然语言，大小写折叠会把"Overview"与"overview"两类不同标题混为一谈，误匹配比漏匹配更难发现；大小写不敏感的文件名匹配（§3.1）是文件系统现实所迫，标题无此约束。备选（已否决）：大小写不敏感。
...
- 创建位置：path 段不含 `/` 时创建于**当前文件所在目录**的 `<target>.md`；当前文件位于 vault 根时即创建于根。path 段含 `/` 时按 vault 根相对路径创建，缺失的中间目录一并创建。（裁决点 I，2026-09-05 Alex 改判定稿：非推荐值"一律建于 vault 根"——短路径链接的意图目标通常与当前文件同域，建于当前目录减少事后移动。）Obsidian 对新建位置有设置项；本 spec 不引入设置，冻结单一确定行为；
