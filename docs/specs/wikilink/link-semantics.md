...
- 以 `#^` 开头的段是块引用（如 `[[note#^abc123]]`）：**不支持**，整条链接标记为"不支持的语法"（§6），不做名称→目标解析；词法分解照常进行——heading path 保留 `^` 段原文并置块引用标记，供显示层给出"块引用不支持"提示。
...
候选全集：`[[...]]` 链接形态为 vault 内全部 `.md` 文件；`![[...]]` embed 形态为全部非目录文件（附件判别见 §5）。按以下顺序，命中即停：

1. **根相对精确路径**：path 段含 `/` 时，按 vault 根相对路径匹配 `<path>.md` 或 `<path>`（path 段自带 `.md` 后缀时等价处理，不重复拼接后缀）。不支持 `..` 与 `./`，含它们的 target 直接判 `unresolved`。
2. **短路径匹配**：path 段不含 `/` 时，按两个子步构造候选集，命中即停：
   a. **完整文件名匹配**（仅当 path 段含扩展名，即含 `.`）：文件名经 §3.1 大小写规则等于 path 段的文件——`[[Beta.md]]` 命中 `Beta.md`，`![[photo.png]]` 命中 `photo.png`；
   b. **去扩展名匹配**：文件名去扩展名后经 §3.1 规则等于 path 段的 `.md` 文件——`[[Beta]]` 命中 `Beta.md`。
   候选集为空 → `unresolved`（未创建链接，§4.3）；恰一个候选 → `resolved`；多个候选 → `ambiguous`，按 ⚠ 裁决点 G 的规则选出 `chosen`。
3. path 段含 `/` 但第 1 步未命中时，按"路径后缀匹配"再走一次候选集逻辑（vault 内路径以 `/<path>.md` 或 `/<path>` 结尾的文件），候选集构造与判定规则同第 2 步。
...
1. target 带非 `.md` 扩展名（如 `![[photo.png]]`），或按 §3.2 解析到非 `.md` 文件 → **附件引用**：附件 SHALL 按**完整文件名**匹配（§3.2 第 2 步子步 a：embed 形态候选全集为全部非目录文件，扩展名参与匹配、不去除——同名笔记 `photo.md` 与附件 `photo.png` 互不抢占）；渲染语义归 attachment 显示能力，本 spec 只管解析与判别；alias 段的展示语义（如 Obsidian 的尺寸写法 `![[image.png|300]]`）同样归附件显示能力，本 spec 不冻结；
...
附件的短路径解析沿用 §3.2：候选全集为全部非目录文件，按完整文件名匹配（第 2 步子步 a）；大小写与歧义规则同 §3.1 与裁决点 G。
