...
- parser 产生 `Table` 后仍须独立验证矩形性。表头、delimiter 和每个数据行按源码槽位恢复后必须列数一致；空槽不能只依赖 `TableCell` 节点，因为 parser 可能不为其创建节点；
- 引用或列表中的表格只在 parser 明确产生完整 `Table` 范围时增强；容器前缀、缩进和原始换行必须保持可选择、可复制；
- 表头/delimiter 列数不一致、数据行少列或多列、范围不完整、槽位不能安全映射，均视为非矩形或不确定表格，整块显示完整源码。系统不得补列、丢列、截断或猜测修复。

源码降级必须覆盖整个候选表格范围，并保持与前后正文的连续选择。它是明确的安全状态，不是把部分 cell widget 与部分源码混排成看似完成的表格。
...
- 表框宽度 = 内容自然宽：每列轨道按 min-content 到 max-content 之间取内容所需宽度，单元格无固定像素上限；空槽与短内容槽保留最小占位宽度。
...
- 反引号内未转义 pipe、表头/delimiter 不同列数、数据行少列/多列、解析中断；
