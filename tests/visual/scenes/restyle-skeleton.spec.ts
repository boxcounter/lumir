...
test("骨架几何：侧栏 236 / 标题栏 42 / modeline 25 / dock 列 0 / 正文列 664 居中", async ({ page }) => {
  ...
  expect(geo.token).toEqual({ sidebar: "236px", titlebar: "42px", modeline: "25px", measure: "664px" });
  ...
  // 正文列 = 定值 664（框宽），两侧等分剩余空间 ⇒ 居中
  expect(geo.scrollerCols[1]).toBe(664);
  expect(geo.contentWidth).toBe(664);
