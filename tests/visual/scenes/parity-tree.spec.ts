...
for (const height of [900, 600, 480]) {
  test(`长目录深层颜色 ${height}`, async ({ page }, info) => {
    ...
    for(const [depth,p] of ['根.md',...dirs.map(p=>`${p}/笔记.md`)].entries()) {
      ...
      const expected=await page.evaluate(({depth})=>{const e=document.createElement('span'); const ratio=[100,96,91,86,81][depth];e.style.color=depth===0?'var(--text)':`color-mix(in srgb, var(--text) ${ratio}%, var(--bg-nav))`;document.body.append(e);const c=getComputedStyle(e).color;e.remove();return c;},{depth});
