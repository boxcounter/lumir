...
test("出厂默认与 Rust `UiConfig` 同值（两处真源对账）", () => {
  ...
  // `CONTENT_WIDTH_MAX`（D1/D2 裁决 2026-09-25 → **2026-09-26 Alex 修订为默认 760、
  // 区间 [760, 1200]**，**默认值即下限**），那边由单测
  // missing_ui_content_width_takes_default / out_of_range_ui_content_width_falls_back_with_warning
  // 钉住同一组值。第三处写值是 src/style.css 的 `--layout-doc-measure` 默认值（CSS 里没法被本层
  // 断言，由视觉场景的默认口径计算属性断言钉住）。REVIEW.md 第 8 条：三处同值。
  assert.equal(DEFAULT_CONTENT_WIDTH, 760, "缺配置时栏宽为 760px");
  assert.equal(CONTENT_WIDTH_MIN, 760);
  assert.equal(CONTENT_WIDTH_MAX, 1200);
  assert.equal(CONTENT_WIDTH_TOKEN, "--layout-doc-measure");
  assert.equal(clampContentWidth(DEFAULT_CONTENT_WIDTH), 760, "出厂默认在合法区间内");
});

test("clamp：区间内原值通过，越界收端点", () => {
  assert.equal(clampContentWidth(760), 760);
  assert.equal(clampContentWidth(1200), 1200);
  assert.equal(clampContentWidth(900), 900);
  assert.equal(clampContentWidth(400), 760, "低于下限收 760");
...
test("对称换算：右缘向右拖 +Δx 放宽 2Δx；左缘向左拖（已带方向的 Δx 为正）同效", () => {
  ...
  assert.equal(nextContentWidth(760, 80), 920, "向右/向外 80px → +160px");
  assert.equal(nextContentWidth(920, -80), 760, "反向拖回");
  // 左右手柄对称：同一 |位移|、同一方向语义（向外为正）给出同一结果
  assert.equal(nextContentWidth(760, 50), nextContentWidth(760, 50));
});

test("取整：小数位移落到整数 px（读数可逐值比对）", () => {
  assert.equal(nextContentWidth(760, 0.2), 760, "0.4px 舍去");
  assert.equal(nextContentWidth(760, 0.25), 761, "0.5px 入（round 半进）");
  assert.equal(nextContentWidth(761, 0.24), 761, "0.48px 舍去");
});

test("拖拽钳制：从默认只能往宽调，触顶后停在 1200", () => {
  assert.equal(nextContentWidth(760, -100), 760, "默认值即下限，往窄拖不动");
  assert.equal(nextContentWidth(760, 1000), 1200, "超出上限钳到 1200");
