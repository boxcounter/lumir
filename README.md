...
| `visual.yml` | 视觉回归**结构层**：DOM 结构、计算样式、可见性与数量、文字内容不回归，另跑套件隔离断言（`tests/visual/isolation.test.mjs`）（CI runner 与本地渲染不等价，整页像素对比归本地 `scripts/gate.sh visual`；口径与证据见 [tests/visual/README.md](tests/visual/README.md)） |
