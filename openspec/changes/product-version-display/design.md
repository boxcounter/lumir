...
- `src/shell.ts`：标题栏 append 第三个子节点 `<span class="titlebar-identity">`，内部分两段
  `<span class="ti-name">Lumir</span>` + `<span class="ti-version">0.0.0</span>`（分隔符「·」
  包在版本段前置或独立 `<span class="ti-sep">`，实现期定——文案组装只在装配层一处）。
...
### 3.2 窗口变窄的退让策略（裁决点 D2 推荐项）

- **优先级：标识 > 标签**。窗口变窄时挤压由 tabstrip 的既有 `overflow-x: auto` 全部吸收：
  标签区可视宽收窄、标签横滚；标识块 `flex: none` + 不截断（MUST NOT 对它用 ellipsis——
  「Lumir · 0.0.0」被截成「Lumir · 0…」比不显示更糟，且 95px 预算没有截断场景）。
- 实测下限：520px 窗口下标签区仍有 272px（≈2.3 个短名标签可视），横滚可达全部标签；
  窗口再窄（<400px）标签区趋窄但功能不丢——macOS 窗口没有 min-width 配置
  （`tauri.conf.json:13-20` 未设），本 change **不新增 min-width**（那是另一个裁决：
  它约束的是整个应用而不止标题栏）。
...
| 窗口阈值下版本号动态退到 modeline | 裁决点 D2 备选：版本号两处展示位（写两处测两处），跨越阈值时元素跳动；标签横滚已覆盖该收益 |
