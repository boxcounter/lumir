...
- [ ] 8.3 `scripts/gate.sh visual` 全绿；基线变动逐张经 Alex 过目后入库　**当前状态（M164）**：视觉门禁的**基线对比部分红**——7 张超容差失败（filetree-open / callout ×2 / mermaid ×2 / render-codeblock / render-quote-list）+ 2 张新增基线缺失（`mv-vault-switcher` 的两张），全部可归因到 M163 的入口形态 A；13 张 Before/After 与 2 张新增已交 tower 转 Alex（`/tmp/wt164-baseline-review/MANIFEST.md`）。**按 AGENTS.md 硬规则与 mission 书，基线在批准前不入库**，因此本条不勾；**批准后**一条命令即可恢复全绿：`pnpm --dir tests/visual test --update-snapshots=all`（或把 `/tmp/wt164-baseline-after/` 覆盖回 `tests/visual/baselines/`）→ `scripts/gate.sh visual` → 再补一次「基线更新」提交。除基线对比外，本轮新增的 9 条视觉用例在 `--update-snapshots=all` 下 **240/240 passed**。
