...
3. **崩溃备份**：dirty 内容持久化到应用恢复目录（`<config_dir>/recovery/`，按 vault + vault 相对路径定位，后端 `recovery.rs`），并记录备份那时的磁盘 revision 作 CAS 基准；保存成功即清除。启动装载 vault 后枚举残留备份，逐个给出恢复提示——「恢复内容」以备份记录的 revision 为保存基准（**不吸收恢复时刻的磁盘 revision**），把备份内容放进编辑器缓冲并保持未保存：备份之后磁盘若被外部修改，随后的保存按 CAS 报冲突，绝不静默覆盖较新版本；「丢弃备份」清理。
