...
| `EditorConfig` **当前只有一个字段** `mode: EditorMode` | `config.rs:72-75`（`mode` 在 `:74`） |
...
| 真实配置形状（验收 harness 写入的即此形） | `scripts/acceptance/lib/app.mjs:28-33`：`{ version, last_vault, editor: { mode }, keys? }` |
...
| 整文件解析失败 → 全部默认 + warning | `config.rs:191-209`；文件不存在不算错 `:173-189` |
...
| 面板场景冻结分组标题数组（9 个，含「标签」）、行数 = `KEY_BINDINGS.length`、未绑定行数 = 0 | `tests/visual/scenes/m133-describe-bindings.spec.ts:15`、`:69`、`:75`；逐命令可见断言 `:73-75` |
...
| `EditorView.lineWrapping` 的实现是给 content 加类：`contentAttributes.of({"class": "cm-lineWrapping"})` | `node_modules/@codemirror/view`（CM 6.x）base theme：`.cm-lineWrapping { white-space: break-spaces; word-break: break-word; overflow-wrap: anywhere; flex-shrink: 1 }`；`.cm-content` 缺省 `white-space: pre` |
| `.cm-scroller` 带 `overflow-x: auto`（CM 基础主题给），本仓主题没取消它 | 本仓 `.cm-scroller` 主题：`src/editor.ts:1129-1135`（md 列 `minmax(24px,1fr) minmax(0,var(--measure)) minmax(24px,1fr)`，code 列首列 `minmax(max-content,1fr)`）；`.cm-content { min-width: 0; width: 100% }` `:1136-1141`；`--measure: 80%` `src/style.css:9` |
...
| `EditorView.blockWrappers` 的契约：facet 值可以是 `RangeSet<BlockWrapper>` 或 `(view: EditorView) => RangeSet<BlockWrapper>`；renderer 遍历**全部** facet 值，故多个 wrapper 函数可并存；`rank` 决定嵌套（小者在内）；wrapper 影响「起点落在其范围内的行/块 widget」；**不许返回 null，跳过 = 从数组里省掉** | `node_modules/@codemirror/view` 的 `index.d.ts`（`blockWrappers` facet 与 `BlockWrapper` 定义）；本仓用法见上一行 |
| 高度纪律：CM 按 border-box 量行高，**margin 对高度图不可见**，会造成 pos↔coords 漂移，故块级容器只用 padding 不用 margin | `src/preview/theme.ts:96-100`、`:206`、`src/style.css:284-288`；表格容器的 `padding-block: 12px` 正是这条下的选择 |
...
- 面板侧还有两处隐含假设：行数 = `KEY_BINDINGS.length`、未绑定行数 = 0
  （`tests/visual/scenes/m133-describe-bindings.spec.ts:69` `:75`），以及「打开面板看到生效表」
  scenario 里那句「无「未绑定」行」（`openspec/specs/keymap-commands/spec.md:288`）。
...
- `sessionState(doc, path, mode)`（`src/editor.ts:1173-1233`）增加折行参数，与 `mode` 同形。
...
2. **块级滚动容器**：新增 `codeBlockWrappers(view)`，与 `tableWrappers`（`src/preview/livePreview.ts:234-279`）
   同形——从语法树取 `FencedCode` / `CodeBlock` 节点的行范围，为每个块返回一个 rank 级低于表格外层
   的 `BlockWrapper`（`div` + `overflow-x: auto` 类）。同 facet 的多个 wrapper 函数并存是 CM 支持的
   （renderer 遍历全部 facet 值），表格那套不受影响。
3. **MUST NOT 拷贝表格容器的 `padding-block: 12px`**：给代码块加块级内边距会改变文档高度、把下方所有行
   推走，而 CM 的高度图按 border-box 量（`src/preview/theme.ts:96-100` `:206`；`src/style.css:284-288`）。
   翻转开关带来的几何变化 SHALL 只来自折行本身。
...
- **模式边界**：`editor.code_block_wrap` 的作用面只有 md 模式（围栏代码块只在 live preview 里渲染）。
  非 md 文件以只读 code 模式打开、没有围栏渲染（`openspec/specs/editor-live-preview/spec.md:11-20`），
  该开关对它们无可观测效果——这不是缺口，是作用面如此，写进 spec 避免被读成漏实现。
...
- 命令 id：`view.toggle-line-wrap`、`view.toggle-code-block-wrap`，进 `NON_TAB_GLOBAL_COMMAND_IDS`
  ...
  - 作用域取 `global` 的理由：用户点了文件树、焦点不在编辑器内容区时，仍应能切换当前标签页的显示；
    ...
    改回 `editor.` 前缀并放进 `EDITOR_COMMAND_IDS`，行为差别只是「哪些焦点下按键有效」。
...
- 面板：两条命令会落进既有「全局」组（`src/bindings-panel.ts:42` 用 `NON_TAB_GLOBAL_COMMAND_IDS`），
  **不新增分组**——分组标题是 `文案-Copy.md` D63–D67 的条目，不新增分组就不动文案。面板会把它们渲染成
  「未绑定」行（既有能力），行数与未绑定行数的断言要跟着改（`m133` 的 `:69` `:75`）。
...
2. **滚动条出现时的水平位移**与「翻转前后除折行本身外行盒不变」的断言取法。
3. **类型不符配置**（`{"editor": {"line_wrap": "yes"}}`）的现行为实测确认（预期走整文件回落），留单测钉住。
4. **现有基线暴露不了本次默认变更**（代码块 fixture 最长行 45 字符）：视觉验收必须自带超长代码行 fixture，
   并逐张核对出现过代码块的整页基线（REVIEW.md 第 3 条）。
5. **`[keys]` 绑定后真的能触发**：需要一条视觉或真机断言（不是只看面板显示改对了）；键位注入类断言按
   acceptance README 的口径走「回读 + 只在字节未变时重试」。
6. 启动早期（配置到达前）与配置到达后的折行状态是否出现可见跳变——文件级折行只在有超长行时可见，
...
| 8 同一语义两处真源 | 折行优先级规则只有一条；TS/Rust 两处默认值按 2.7 的处置（两侧测试 + 指针注释） |
| 9 声明即被消费 | 两个配置项都有消费者（启动应用 + toggle）；`KEYLESS_COMMAND_IDS` 有消费者（单测 + m131 对账），不留假开关 |
