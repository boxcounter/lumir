...
2. **方向 A 落地**：模式裁决对 `.md`/`.markdown` 返回 md，对**其余一切已打开的文件**返回**只读 code 模式**（已知代码扩展、`.txt`/`.log` 之类的未收录扩展、dotfile，以及 basename 无点的 `LICENSE`/`Makefile`），MUST NOT 回落配置默认；配置 `editor.mode` 自此只对「没有文件上下文」的文档（空态 / 新建）有意义。于是 `.php`/`.svelte`/`.txt` 等打开即只读，dirty 不可能产生，保存死态在两个方向（配置默认 md 或 code）下都不再出现。
   - 无扩展名文件是否也收归只读，是本次任务书里唯一自相矛盾处（task 2 既写「ext 非 md/markdown 时不 fallback」又写「ext 缺失/未知保持现状 fallback」）。M130 评审经 tower 裁决：**Context 里 Alex 的原话「非 md 即只读」（D4）优先**，任务书那句「保持现状 fallback」按笔误处理——无扩展名文件一并只读。
3. **不可保存态的可见反馈（兜底防再犯）**：dirty 且当前文档没有落盘能力（未打开文件 / 非 md 模式 / 未登记磁盘 revision）时，Cmd+S MUST 给出可见反馈并指出脱离 dirty 的动作（撤销修改），MUST NOT 静默 return；切换文件 / 切换 vault 的 dirty 守卫提示同样不再建议走不通的「请先保存（Cmd+S）」，改指撤销修改；崩溃备份对无落盘基准的 dirty 内容改为**显式跳过**并记录裁决（不写无法闭环的备份）。方向 A 收口后该兜底当前可达的触发路径只剩「没有打开文件」（空态 / 新建文档）；非 md 只读分支保留为防再犯的防线。
4. **回归证据**：新增 `tests/visual/scenes/m130-text-open-trap.spec.ts`——非 md 只读矩阵（未知扩展 / 差集扩展 / dotfile / 无扩展名文件）、差集扩展的语言归属、无落盘基准时的两处可见反馈（Cmd+S 与切换守卫）；视觉 stub 为既有非 md 条目补最小内容能力，使这些场景能点到真实文本而非 `fs_not_found` 提示。
...
- 不改 `editor.mode` 配置项本身：它继续决定「没有文件上下文」文档（空态 / 新建）的模式，只是对打开文件不再有任何影响；`config.rs` 的字段、出厂值与校验逻辑本次不动（该文件不在本 mission scope）。
