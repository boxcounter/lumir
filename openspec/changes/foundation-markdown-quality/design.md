...
P0 组合是基础出口。P1/P1.5 进入 Foundation 的实现顺序，未完整实现时必须展示明确降级，不得出现无限 loading。P2 仅保留 dataview/canvas，记录为延后且需单独产品裁决。Math（LaTeX）与 Mermaid 按用户裁决为确定需求：必须渲染为可读图形结果；解析或渲染失败时降级为可读源码或明确失败状态，不伪装已支持；阅读、选择、复制输出的原始源码保持不变；其渲染作为异步资源纳入 F1，不阻塞 F0。列表与 pipe table 优先复用 parser 结构信息。非矩形表格回退可读源码，不猜测修复；宽表使用局部可聚焦横向滚动区。
