...
- [x] 6.1 新增场景 `scripts/acceptance/scenarios/40-table-fullscreen-view.md`（编号声明见文首；
      ...
      **r1 评审 P2-1 的修复**：负对照原先只有 `clickEditor`（caret 定位）、**从未按下 ⌘J**，
      两条断言在「命令没发出」时也恒真，步名与「判据口径」段的「执行命令」措辞构成 REVIEW.md 第 6 条
      的覆盖虚标；已按评审修法拆成「负对照·定位」（断言焦点在编辑器正文里，命令发得出去的前提）+
      「负对照·执行」（`do: key cmd+j` + 遮罩未出现 + 文档逐字节不变 + 现场截图）并同步措辞，
      重跑 `run.mjs 40` = **1/1 PASS**（30 条断言全 PASS，7 张截图，序号与本次运行一致——上一轮的旧截图已从本轮目录清掉，修前整套记录留在 `…-prefix-r1/`）；
      修前那份 PASS 记录留档在 `…/2026-09-26-m240/40-table-fullscreen-view-prefix-r1/` 供对照。
