...
2. **yaml 的「看起来没高亮」用 Alex 自己 vault 里的实际块复现了。** 取 `/Users/boxcounter/Downloads/Everything-copy/4_Archives/Engineering/Logbook/README.md` 的第一个围栏块（```yaml，28 行，`dimensions:` 那张表）原样渲染：**14 个着色 span 里 13 个是键，全部落同一个类** `cm-lp-tok-literal`（`rgb(160, 94, 28)` 赭色，与数字、布尔同色），余下 1 个是注释；`property` / `string` / `keyword` 三类**零命中**。值（`Goal`、`monthly`、`Slax Reader`…）、`-`、`:` 一律正文色。**整块只出一种颜色**——这正是 M147 修 json 时 Alex 说的「全是一种颜色，和没着色没有区别」，只是换成了 yaml。截图 [`evidence/01-yaml-block-as-is.png`](evidence/01-yaml-block-as-is.png)（实现前现状），逐 span dump 与命令见 [`evidence/03-复核记录.md`](evidence/03-复核记录.md)。
