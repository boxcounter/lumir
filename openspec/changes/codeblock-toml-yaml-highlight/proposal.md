...
### 裁决记录（2026-09-18，节点 1）

- **裁决点 1：授权 worker 判断**（Alex 原话「这个按照 worker 的判断来，我没有异议」）。落定两件事：
  1. **yaml 的单色缺陷独立成立**——它是 vendored parser 与配色表的既有口径问题（`atom` 落字面量组），
     与实例载入的是哪个构建无关，`evidence/01` 已复现。**本 change 不撤回**，第 2 组起照方案实现。
  2. **toml 的现场采信假说 (b)**：toml 在可触及语料里零现场（`evidence/03-复核记录.md` §4），
     而主仓 `dist/` 停在 2026-09-16 15:42、早于 M138 的 20:53，那份前端里 `cm-lp-tok-` 命中数为 0
     ——走打包 / 裸二进制起实例时任何语言都不会着色。据此 toml 按「无产品缺陷、只纳入门禁」处置，
     不做任何取色改动。
- **裁决点 2：方案 A**（Alex 原话「采纳你的建议」）。只把 yaml 的键改取属性色：不改配色角色表、
  不改 toml、不新增颜色、既有整页基线零变更。**方案 B 不实现**，归档时标注放弃原因（证据见
  `evidence/03-复核记录.md` §6：给 `meta` / `punctuation` 加角色会同时改掉 17 / 21 门收录语言的观感）。
