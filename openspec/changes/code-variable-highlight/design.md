...
本文件分九节。**第 1 节与同批 [code-outline 的 design](../code-outline/design.md) 的第 1 节逐字一致**（同一份语言栈盘点的两条消费者）；第 2–9 节是本 change 自己的。本 change 的判据全部落在解析树上，判据的第一手实测（双击选区、逐语言父链转储）见 [evidence/03](evidence/03-identifier-positions.md)。
...
- 结构解析器注册表、解析触发与缓存由 [code-outline](../code-outline/proposal.md) 引入（`src/code-structure.ts`；spec 落 `editor-live-preview` 的「代码文件的结构解析（语言分层注册表）」）。
...
- 失效：换文件 / 外部重载 / 切标签 → 丢索引；从未触发过的文件不建索引。
...
- 与「文件内搜索只看视口」的既有纪律（in-file-search spec 的「高亮只看视口」）不同：本 change 的高亮**是全文的**（绑定出现位置本身是全文概念）。这条差异写进 spec，避免日后被读成违规。
...
- 取色：**只取既有 editorial token**，MUST NOT 新增颜色（与 json/yaml 键色修复同一条硬约束）。推荐底纹 = `--bg-2`（既有 token，`src/style.css:6`；toc 的 `is-active` 已用它表达「次一级的选中底」），不改字色、不改字重、不加下划线（字色已被 token 着色占用）。
...
| 绑定匹配（本 change） | `--bg-2` 底纹 | 新增（取既有 token） |
...
- **未实测的声明位形态**（实现期须逐条补断言或如实标为不支持）：python 的 `with … as` / `except … as` / 解构赋值 / 推导式目标 / 模式匹配绑定；js 的解构（`const { a } = obj`）与 `import` 绑定；c/cpp 的含糊声明（`a * b;`）；go 的 `range` 目标与多返回值赋值；rust 的模式绑定（`let (a, b) = …`）。
- 1MB 级文件的索引内存与装饰重建耗时（§4.2 / §4.4）——headless 只有量级读数，须真机复测。

**未验证项（不许当成已验）**：

- 真机（WKWebView）双击 → 装饰出现的可感延迟与手感；`--bg-2` 底纹在真实屏幕上的可辨识度（归 Alex 的手感项）。
- 真实 vault 语料上的误亮率：判据用固定小语料逐例实测过（[evidence/03](evidence/03-identifier-positions.md)），**没有**跑过真实文件集的抽样统计。实现期须给一份抽样读数（取 vault 里 N 个代码文件、人工核 M 个触发），把误亮/漏亮形态如实记录。
...
- 本 change **依赖** [code-outline](../code-outline/proposal.md) 引入的「代码文件的结构解析（语言分层注册表）」。
