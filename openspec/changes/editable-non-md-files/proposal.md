...
> M231：D1 全量文本类 / D2 code 模式 / D3 保存全量复用 / D4 维持既有护栏（全推荐）

即：D1 可编辑范围 = 注册表全量文本类（md / code / text 三族）；D2 编辑形态 = code 模式编辑；
D3 保存链路 = 全量复用现有 `saveDocument` 链路；D4 护栏维持既有（二进制点击「暂不支持预览」、
非 UTF-8 拒读 `fs_invalid_utf8`、超 50MB 拒读 `fs_too_large`）。

四项**全部采纳推荐项**，本节因此是「授权实现」而非「待决」：下面的 options 表逐字保留原始论证，
仅作历史档——delta（`specs/**`）与清单（`tasks.md`）起草时已按推荐项写就，实现按本记录施工。

**标注**：原话只给出四个代号 + 「全推荐」；上面「即：…」一行是**转写**（把代号展开成 delta 里的
具体口径），不是原话。两段分列的理由：本仓的裁决记录以**原话**为凭据，转写若与原话有出入，
以原话为准。

## 须提请 Alex 节点 1 裁决的选项（已于 2026-09-25 全部落槌，结果见上「裁决记录」；本节保留原始论证）
