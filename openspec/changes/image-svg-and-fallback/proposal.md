...
| 选项 | 内容 | 代价 |
|---|---|---|
| **A（建议）** | 维持现状：本 change 不加图片行的显露，把「图片行是否与链接/分隔线同口径」单独立项 | 本 change 的改动面收在最窄；「看不到源码」由可见占位（含原始引用文本）缓解——占位上就写着路径与 alt |
| B | 并入本 change：`Image` 分支加选区判断，图片行照常显露源码 | 同一物理动作（点图片）会让图片消失、源码出现，是对**所有**图片的可见行为变更，超出 Alex 本次报告的诉求面；需同步改 `editor-live-preview` spec 的显露覆盖集枚举 |
