...
- [ ] 1.1 真机复现 Alex 的原始形态：在验收 vault 放一份**带 `width`/`height`** 的 svg，用
      `![Hooks Overview](images/hooks-overview.en.svg)` 引用并打开该 md，记录终态属于哪一类
      （正常显示 / 可见占位 / 空白）。
      **验收口径**：截图 + AX dump 落 `test-results/acceptance/<日期>/<场景>/shots|ax/`；终态归类写进
      实现 PR 说明。
- [ ] 1.2 复现「渲染尺寸为零」样本：放一份只声明 `viewBox`、不声明宽高的 svg（以及一份同形态的
      位图不需要），打开后记录终态。
      **验收口径**：该样本在**修复前**必须观测到「源码被替换 + 无可见内容」（即空白），截图留档。
      这一条同时是第 5.2 条反向验证的输入。若观测结果是可见占位而非空白，说明第 3.2 节的终态判据
      选点错了，须回到 design §3.2 复核后再动实现。
- [ ] 1.3 判定真实机制：对 1.1 与 1.2 的样本，区分「`<img>` 抛 `error`」与「解码成功但尺寸为零」
      两类（手段不限：临时探针、WKWebView 远程调试、或按 `naturalWidth`/替换区尺寸双读数）。
      **验收口径**：把结论回填 design §8 的「未定」行，并写明判定所用的读数；两类的兜底路径不同，
      结论直接决定 2.1 的实现取点。
