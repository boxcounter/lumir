...
Alex 节点 1 裁决，原话：「**B 全推荐**」——上表四个裁决点全部取推荐项，delta 与 tasks 无需改写
（推荐项的形态**从一开始就是按默认写进 delta 的**）。逐项取值：

| # | 裁决取值 | 落地含义 |
|---|---|---|
| 1 | 应用内全屏遮罩 + 图片居中 | 不调外部程序、不开新窗口；遮罩定位在文档流之外 |
| 2 | 适配遮罩且不放大 | 不设 `width`/`height`（小图按自然尺寸）；越界由 `max-width/max-height: 100%` 挡住 |
| 3 | 不加命令、不绑键 | 双击是唯一打开路径；**零** `keymap-commands` 改动、零 `KEYLESS_COMMAND_IDS` 条目 |
| 4 | 三条关闭路径 + 不加提示文案 | `Esc` / 点击遮罩 / 遮罩内再次双击图片；**零**新 deck 条目（`文案-Copy.md` 不改） |

因此 [tasks.md](tasks.md) 的三条条件项按「**节点 1 未选中**」处理：3.4（命令族）与 9.2（键盘打开
路径）不做且如实记为边界；2.6 与 5.6（缩放口径）按推荐项「适配不放大」实现与断言。
