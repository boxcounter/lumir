...
2. **列表项缩进编辑行为**（delta：`keymap-commands`，行为规格与键位同一条 requirement，
   ...
   `ListItem` 连同其续行与子树**整体平移一层**——步长按语法树取该层标记的内容列宽
   （`- ` → 2、`1. ` → 3、`10. ` → 4，实测依据与坐标口径见 design §3），只动行首空白与
   **有序项的源码编号**（D2c 裁决：按新归属重排为规范序号），任务标记不动；该列表项
   没有上一同级项（是所在列表的第一项）时无操作。整次平移是一次 dispatch、一步撤销
   （⌘Z 一次回到平移前）。渲染自动跟随：列表标记与悬挂缩进由 `src/preview/lists.ts` 的
   既有组扫描按新语法树重算，本 change 不动渲染层。
...
### 裁决结果（Alex 节点 1 裁决，2026-09-26，原话逐字）

「D1：a. 绑给列表缩进（editor 作用域）；D2: c. 缩进后重排有序列表源码编号；D3: a. 无操作；
D4: a. 无操作（吞掉）；D5: a. 只作用 head 所在项」。

按裁决落地的改写（上面的「待 Alex 裁决」表保留原选项与理由，**推荐列不代表裁决**）：

- **D1 = a**（与推荐一致）：TAB / SHIFT+TAB 绑列表缩进，scope editor；编辑器内 TAB 的原生
  焦点遍历被接管是知情代价。
- **D2 = c（推翻推荐 a）**：缩进 / 凸排后有序列表源码编号**按新归属重排为规范序号**。已按此
  改写 What Changes 第 2 条、design §3 与 `specs/keymap-commands/spec.md` 的「MUST NOT 重排」
  句与场景——原稿「编号不重排」是被否决的推荐口径，留着会让 spec 与实现当场对不上。
- **实现期实测修正（两条，随本次实现一并上报，节点 2 可一并裁决）**：
  1. **缩进步长按语法树取**（= 该层标记的内容列宽），不是固定 2 空格——固定 2 空格在有序列表
     上不构成嵌套（实测见 design §3），TAB 会退化成不可见空白 diff，D2c 的「新归属」也就无处
     可重排。D2c 的前提正是「缩进一层 = 成为上一同级项的子项」，故按裁决的意图落地。
  2. **无上一同级项（列表第一项）时 TAB 无操作**——没有可嵌套的父项，写入只会留下不可见
     空白 diff（与 D4c 被否决的同一理由），对称于 D3a（顶层项凸排无操作）。
