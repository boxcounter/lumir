...
用户需求原话（2026-09-24，verbatim）：「代码文件也支持 Outline，并且支持双击选中变量后高亮变量（不是字符匹配，要语法上的变量）」。本 change 只做前半句（字符级高亮见同批提案 [code-variable-highlight](../2026-09-24-code-variable-highlight/proposal.md)）。
...
**归档顺序**：同批提案 [code-variable-highlight](../2026-09-24-code-variable-highlight/proposal.md) 依赖本 change 引入的这张表。两份 change 都只对自己 capability 做 ADDED（requirement 名不重叠），互覆风险为零；但**实现顺序**上建议本 change 先行（见那份 proposal 的同一节）。

## 须提请 Alex 节点 1 裁决的选项

本表七项都给了推荐项；推荐项已按默认形态写进 delta 与 tasks，裁决若取备选，实现前先按裁决改写 delta 与 tasks 的条件项，不静默按推荐项做。
...
**节点 1 的四项既定裁决面与本节对应关系**：① 语言覆盖分层 = 裁决点 1；② 新增 `@lezer/*` 依赖许可 = 裁决点 2；
③ md 模式是否同样支持 = 裁决点 4（md 已支持，本 change 是它的扩展；反向问题见裁决点 3）；④ 变量高亮的语义精度
（严格作用域分析 vs 近似）**不在本 change 的裁决面**——它属同批 [code-variable-highlight](../2026-09-24-code-variable-highlight/proposal.md)
...
Alex 节点 1 原话：**「采纳」**（对七点裁决点整体作答）。七项**全部**按上表「推荐（已写进 delta）」列
落地，无一项改取备选：

| # | 裁决点 | 日期 | 原话 | 落地形态（本次实现所依） |
|---|---|---|---|---|
| 1 | 语言覆盖分层方案 | 2026-09-24 | 采纳 | 按两条可复核判据分层：T1 符号大纲 8 门（javascript / typescript / python / rust / go / c / cpp / java）、T2 结构大纲 2 门（css / scss）、T3 不支持 11 门（含 **shell / yaml 无大纲**，且**不做任何文本级降级**） |
| 2 | 新增 `@lezer/*` 依赖许可 | 2026-09-24 | 采纳 | **许可接受**：装 8 个官方语法包（T1 + T2），逐包 license 与体积读数落实现期证据；不引入任何第三方配色主张（不消费语法包的 styleTags） |
| 3 | 代码大纲与 md 大纲的异同 | 2026-09-24 | 采纳 | 异：跳转落点取**声明起点**、层级取**语法嵌套深度**、空态**新增两条文案**（缺条目 / 语言不支持）；同：同一指示段、同一浮层、同一组就地键、同一跳转语义 |
| 4 | md 模式是否同样支持 | 2026-09-24 | 采纳 | md 已支持，本 change 是它的扩展；md 侧全部既有口径逐字不动 |
| 5（附） | `php` 是否顺带接上 | 2026-09-24 | 采纳 | **不接**，维持 M130 的「无语言包 = 纯文本」 |
| 6 | 机制选择：结构解析与着色管线的并存形态 | 2026-09-24 | 采纳 | **S2 双管线**：legacy mode 继续当着色来源（`language` facet 不动），结构解析用 `@lezer/*` 自己驱动、按需一次并缓存。**parity 不变量（围栏与 code 模式同 tag 同配色）因此逐字照旧**；常驻内存代价须在真机端点复测，不达标不得宣称 |
| 7 | 指示段的出现时机 | 2026-09-24 | 采纳 | **惰性**：结构未解析时指示段不显示，且**不为了点亮它而触发解析**；解析只在用户显式动作（`⌘⇧O`）时发生。打开路径与光标路径都零解析 |

**因此 tasks.md 的「条件项」一节一条都不需要改写**：七项全落推荐列，作用面、语言集合、落点口径与
tasks 原文一致。那一节按原样保留——它记的是「若将来改取备选时如何改写」的路径，不是本次要做的事。

**实现期的文档对齐两处**（不触碰裁决面，只把两处内部不一致改准，见本次实现的偏离申报）：

1. **不支持语言的提示文案以 delta 为准**：specs/toc-outline 的 delta 写的是「这份文件类型暂不支持大纲」，
   design §2.4 / §3.2 写的「这份文件的语言暂不支持大纲」与之不一致——实现取 **delta 的措辞**，
   design 两处同步改成 delta 的用词。
2. **design §2.1 的类别词表里 `module`（命名空间）在 spec 的逐语言覆盖表里没有任何消费者**：
   按 [REVIEW.md](../../../../REVIEW.md) 第 9 条（声明了却没有消费者），实现的类别联合里不收 `module`；
   将来若要 namespace / `mod` 条目，须先改 delta 的覆盖表。
