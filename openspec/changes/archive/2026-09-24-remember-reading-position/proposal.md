...
Alex 节点 1 原话：**「都采纳」**。四个裁决点**全部**按上表「推荐（已写进 delta）」列落地，无一项改取备选：

| # | 裁决点 | 日期 | 原话 | 落地形态（已实现并实测） |
|---|---|---|---|---|
| 1 | 记什么 | 2026-09-24 | 都采纳 | 只记滚动位置（文档位置锚 + 纵向视口偏移 + 横向滚动量）；不记光标 / 选区 / 撤销史 |
| 2 | 适用面 | 2026-09-24 | 都采纳 | md 与 code 两种模式都记；两模式的往返都有实测（design §7 第 1、6 条） |
| 3 | 条目保留与失效 | 2026-09-24 | 都采纳 | 每 vault 上限 200 条、按写入时刻最旧淘汰；装载后按本次枚举清理；打不开的键只跳过不删；改名 / 移动视为丢失；版本不符或解析失败等价于无历史 |
| 4 | 与 backlog #10 的关系 | 2026-09-24 | 都采纳 | 只补 #10 留下的「滚动位置」一格：不扩 `vault-sessions/<id>.json`、不做光标 / 撤销史恢复、不动 #10 的其余设计 |

**因此 tasks.md 的「条件项」一节一条都不需要改写**：四项都落在推荐列，作用面、上限取值、清理时机、
落点与 tasks 原文一致。那一节按原样保留——它记的是「若将来改取备选时如何改写」的路径，不是本次要做
的事（tasks 开头已按此标注）。

**实现期对 design 的两处实测修正**（不触碰裁决面，只改技术推导，逐条落 design §7 与实现注释）：
纵向偏移取「相对滚动容器」而不是「相对文档顶」；远端锚处 `coordsAtPos` 返回 null 时不得放弃恢复。
