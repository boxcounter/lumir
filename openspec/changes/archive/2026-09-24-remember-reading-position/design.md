...
- **实测修正（2026-09-24，见 §7 第 2 条）**：上面这条推导有两处错，实现期被实测读数钉住并改掉了——
  ① CM 的 `documentTop` 是 `contentDOM.rect.top + paddingTop`（**随滚动平移**的文档原点），因此
  「字符盒顶 − documentTop」与 scrollTop 无关；`y` 必须取「字符盒顶 − **滚动容器顶**」（视口偏移），
  否则恢复落点恒为 44px。② 恢复**不能**用 `coordsAtPos` 做前置可读性门槛（装载刚结束时锚几乎总在
  视口外、返回 null），直接发放效果即可。选型（自分解 + 只用文档化 API）与「捕获与恢复同一口径」
  这条纪律不变。
...
## 7. 未验证项与已知边界（**实测结果**，2026-09-24 实现期）

本节原稿列了 9 条「实现期 MUST 实测」的项，并明确「不许当成已验」。下面是**实测结果**：读数与断言
在 `tests/visual/scenes/reading-position-probe.spec.ts`，原始读数按测试名分文件落在
`test-results/m194-reading-position-probe/design7-probe/*.json`（本机留存、git 外）。

| # | 项 | 结论 | 实测读数 |
|---|---|---|---|
| 1 | 往返精度 | **已实测** | 同一处「捕获 → 关标签 → 重新打开」：`scrollTop` 6160 → 6160（差 **0**），顶部可见行逐字相同（`第 218 行`）；装载后 **120 帧逐帧采样值全部相同**（无人在恢复之后改它） |
| 2 | `view.documentTop` 的坐标系 | **已实测，并推翻了原稿口径** | `documentTop = 滚动容器顶 − scrollTop + paddingTop`（scrollTop=0 时 132 = 88 − 0 + 44）⇒「字符盒顶 − documentTop」是**与滚动无关的常量**（6163），拿它当 yMargin 时恢复落点恒为 `paddingTop`=44px（页首附近）。改取「字符盒顶 − **滚动容器顶**」后往返精确。见 §2.3 的修正与 `src/scroll-position.ts` 的推导 |
| 3 | 含异步加载图片的文档 | **已实测** | 图片在锚上方、字节延迟 500ms 到达：锚的视口偏移 101 →(装载后、字节未到) 101 →(字节到达) 101；顶部可见行三次都是 `第 096 行`；`scrollHeight` 5156 → 5240 → 5156（高度确实变过而参照行没漂） |
| 4 | 栏宽变化后再打开 | **已实测** | 视口宽 956 → 696（栏宽是百分比）：`scrollTop` 6110 → 6110，锚的视口偏移 97 → 97，顶部可见行逐字相同 |
| 5 | 页面缩放（`scaleY != 1`） | **已实测（本仓为 1）** | `view.scaleY = 1`、`documentPadding.top = 44`；全仓无 `zoom` / `transform` ⇒ 两条分支同值、闭环 |
| 6 | code 模式的横向偏移 | **已实测，且换掉了恢复通道** | 折行关闭（`line_wrap: false`）：`scrollLeft` 200 → 200（逐像素）；前提条件也是实测的（`scrollWidth` 4173 > `clientWidth` 956）。**修正**：横向不能走 `scrollIntoView` 的 `xMargin`——非快照分支先减 `getScrollMargins(view).left`，本仓 gutter 正好提供它，实测系统性偏 **34px**（= gutter 宽）；改与 CM 快照分支同口径（`x` = 原始 `scrollLeft`，恢复时直接写 `scrollDOM.scrollLeft`，在效果落地后的下一帧写以确保它是最后一次写入） |
| 7 | 文件在两次会话之间被外部改写 | **已实测** | 锚落在文档之外（`pos` 超出文档长度 10 万）时夹到末尾：`scrollTop` 7864 = `maxScrollTop` 7864，无任何提示；不做内容比对（有意选择，原稿不变） |
| 8 | 文件改名 / 移动 | **已实测** | 位置记在 `old.md`、文件以 `moved.md` 出现：`scrollTop` 0、顶部可见行 = 文档首行（键是路径 ⇒ 改名即丢失，原稿结论不变） |
| 9 | 多窗口 | **未实测（静态事实 + 推断，如实标注）** | 本仓单窗口（`src-tauri/tauri.conf.json` 只有一个 window），写是 tmp+rename；「后写覆盖先写」没有并发实测（单窗口下构造不出来），不作为已验 |

### §7 之外的两处实测发现（原稿未列，实现期踩到并已修正）

- **远端锚的 `coordsAtPos` 返回 `null`**（实测：`scrollTop` 3366 时锚在视口下方约 2700px，读数为
  `coordsNull: true`）。§2.3 的兜底原本写「`coordsAtPos` 返回 null 时放弃本次恢复」——照此实现会让
  **所有深于一屏的位置永久失效**（装载刚结束时锚几乎总在视口之外）。修正：恢复不做前置门槛，直接
  发放效果（CM 自己先按 scrollTarget 重定位视口、渲染、测量，再算坐标；锚真的不可读时 CM 的
  `scrollIntoView` 自行提前返回 ⇒ 静默退化到篇首，仍是 spec 要的行为）。
- **搜索面板在场时**：CM 的 PanelGroup 会提供 `scrollMargins.top`（面板高度），它只影响
  `scrollIntoView` 的测量口径，因此**面板打开期间**的纵向恢复可能偏一个面板高。本 change **不补偿**
  （面板是暂态，且补偿要读 CM 的内部边距、不是公开 API）——这是**读源码得出的机制，未实测**，
  如实记为已知边界，不写成已验。
