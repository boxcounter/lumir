...
- [x] 8.1 **新增场景** `scripts/acceptance/scenarios/29-typography-and-zoom.md`（**实现期实测偏离**：本表
  ...
  master；实测 `ls scripts/acceptance/scenarios/` 的最大编号是 28，故就地为 29、`item: 29`；另本表 ①② 原写 `font_size: 20`，实测场景用 **32**，归档时按场景正文改准）：①以配置 `font_size: 32` 启动 → 读 AX / 几何证据确认字号生效（配置通道的端到端）；
  ②按 `⌘=` / `⌘−` / `⌘0` → 字号读数逐档变化、`⌘0` 回到配置值 32；③按 ⌘⇧=（真机的 `⌘+` 形态）确认同样放大；
  ④`config.json` 内容与 mtime 逐字节不变。
  **验收口径**：`node scripts/acceptance/run.mjs --check` 通过（M195 时点 36 个场景全通过；2026-09-24 归档复跑 39 个场景全通过）；
...
| 2.4 | `scripts/acceptance/lib/app.mjs` 的 `writeConfig`（+`lib/execute.mjs` 的 front-matter config 与 `configWrite` 透传） | `node scripts/acceptance/run.mjs --check` → 全通过（M195 时点 36 个场景；2026-09-24 归档复跑 39 个） |
...
- 场景文件 `scripts/acceptance/scenarios/29-typography-and-zoom.md`（编号按现有最大 28 续号 → **29**，
  ...
  `mtimeUnchangedSince` 断言、front-matter/configWrite 的透传）已提交，`--check` 全通过（M195 时点 36 个场景；2026-09-24 归档复跑 39 个）。
