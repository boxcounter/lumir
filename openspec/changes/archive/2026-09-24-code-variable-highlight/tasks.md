...
实现待 Alex 节点 1 裁决后另立 mission；实现顺序建议在 [code-outline](../2026-09-24-code-outline/proposal.md) 之后
...
- [x] 5.1 新增 `src/code-identifiers.ts`：从解析树构建名字 → 出现位置 / 位置类别 / 容器链的索引。
  **验收口径**：索引构建的耗时读数落 `test-results/m198/perf-var-highlight.json`（100KB 与 1MB 各一份
  读数；原稿写的 `index-cost.json` 不存在，归档时改准）；
...
- [x] 5.4 性能与内存读数（真机 / 产品端点）：大文件（1MB）+ 高频标识符的装饰重建耗时、索引与树的常驻占用。
  **验收口径**：读数落 `test-results/m198/perf-var-highlight.json`（原稿写的 `perf-var-highlight-*.json`
  系列不存在——实际是单份 json、内含 100KB 与 1MB 两组读数，归档时改准）；**不达标就写不达标**，并按
...
- [x] 8.1 **编号已顺延为 31**（`ls scripts/acceptance/scenarios/` 的现有最大是 30，30 是 code-outline）：
  ...
  **口径就地修正（2026-09-24，tower 裁决采纳；原因：本组的原字面要求在真机通道上不可判）**：
  ...
  全部落 chromium 层。**测量指针**：`test-results/m198/task8-real-machine-scope.md`（覆盖矩阵；触发前后
  AX dump 逐行比对的「无可判信号」一节在**同一文件的第三节**——原稿写的 `task8-ax-probe.md` 不存在，
  归档时改准）。
  **验收口径**：`node scripts/acceptance/run.mjs --check` 通过 + 真机 PASS，证据落
  `test-results/acceptance/<日期>/31-code-variable-highlight/`（场景编号在实现期由 29 顺延为 31，原稿
  写的 `29-code-variable-highlight/` 已改准）。**真机判据限制**：AX 不暴露装饰与颜色，
...
- [x] 9.4 收官对账：tasks 全部勾选（或标注放弃原因）；spec 增量与实现逐条对账（三层判据、排除面、保守
  方向、三层可区分、生命周期）；design §7 的未验证项按实测改写——**实现期未做**（`c21c780` / `5ce661a`
  两次实现提交都没碰 `design.md`），2026-09-24 归档提交（M205）补齐：§7 的「未实测的声明位形态」改为按
  M198 实测收口的写法（含残余少亮 / 混组清单的指针），`--bg-2` 三处按实现的 `--bg-3` 改准，误亮率一条
  补上 M198 的抽样读数与它自陈的边界。
...
| 5 索引与缓存 | 复用 M197 解析（一次解析）；缓存身份 = 文档内容 | `src/code-identifiers.ts` 的索引缓存；`src/code-structure.ts` 的 `structureTree` | 单测「共用解析」「索引按文档缓存」；`perf-var-highlight.json`（**1MB 首触发 648.4 ms = 解析 205.8 + 索引构建 442.6，不达标**；超标归因 = 索引构建，后续路径 design §8 备选②） |
...
| 8 真机验收 | （口径已就地修正）可判三条 + 截图 | `scripts/acceptance/scenarios/31-code-variable-highlight.md` | `acceptance-31-run4.log`（**1/1 PASS**）、`task8-real-machine-scope.md`（其第三节 = 触发前后 AX dump 逐行比对、结论「无可判信号」；原稿此处写的 `task8-ax-probe` 文件名不存在，归档时改准） |
| 9 门禁与收官 | gate / docs-check / openspec validate 全绿 | — | `gate-visual.log`（visual 档 = quick + 视觉回归，其中含 `GATE PASS docs-check` 与 `GATE PASS openspec-validate` 两行；原稿并列的 `gate-quick.log` / `docs-check.log` / `openspec-validate.log` 三个文件不存在，归档时改准）；`false-positive-sample.md`；`docs/backlog.md` 第 22 条 |
...
**如实登记的不达标项**：1MB 级文件的首次触发耗时（`perf-var-highlight.json` 落盘读数：解析 205.8 ms + 索引构建 442.6 ms = **648.4 ms**）超过 design 的量级估计；**超标归因是索引构建**（解析 205.8 ms 落在 design §1.6 的 78–291 ms 估计区间内，而索引构建 442.6 ms 远超该处 6–8 ms 的树遍历估计）。后续路径按 design §8 备选②（只留派生数据、丢弃树）。（2026-09-24 归档时按落盘读数改准：本行与对账表原写「0.91s（解析 0.28s + 索引构建 0.64s）」并称超出的是解析量级估计，两处与读数不符。）
