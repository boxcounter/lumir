...
| 8 | **已实测（chromium，r1 复审后补）**：原先只登记「结构证据」，r1 评审走查证明那套证据**不成立**——浮层容器对单字符走「其余键不消费」，事件照常冒泡到 window 分发器；用户绑定派生的 scope 是 **global**，分发器只对 editor 作用域判 `isEditorEvent`，于是 `preventDefault()` + 保存触发：**字符进不了输入框**，与 delta scenario 的 THEN 恰好相反。修复（tower 裁决路径 ①）：`src/keys.ts` 的 `Keymap.handle` 加**可编辑宿主守卫**——事件目标落在编辑器之外的可编辑宿主（原生 input / textarea / contenteditable）时不分发任何绑定、不 `preventDefault`（编辑器 contentDOM 排除在外，判据沿用既有的 `ctx.isEditorEvent`）。断言落 `tests/visual/scenes/editable-host-guard.spec.ts`：配置 `s = document.save` 后 ① 筛选输入框里打 `s` → 字符落进输入框（`toHaveValue("s")`）、桩的 `document_save` 调用数不变、无「已保存」浮条；② 浮层外（编辑器）打 `s` → 保存真的落盘（写盘记录 +1、内容含 `x`）；③ 搜索 panel 侧同型一条。**反向验证 RV-5**：关掉守卫后两条都红（读数 `Received: ""`——字符根本没落进输入框），即复现了评审描述的那条路径。副作用（有意、已申报）：守卫覆盖**所有**非编辑器可编辑宿主，搜索 panel 输入框里按 ⌘S/⌘F 等 chord 也不再触发绑定。真机侧同族现象（字符进输入框）由场景 32 的 AXValue 断言覆盖。 |
