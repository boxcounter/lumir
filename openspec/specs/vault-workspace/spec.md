...
失效行后**点击**它的动作 SHALL 保持现状（走重定位，不发起切换）——该行不在键盘游标空间里，键盘路径
到不了它（见下方 scenario）。
...
#### Scenario: 失效行参与筛选但不进键盘游标空间

- **WHEN** 某失效 vault 的显示名命中查询
- **THEN** 该行 SHALL 出现在结果集里（可点击、带既有失效标记），但 MUST NOT 进入键盘游标空间——键盘
  游标只在可用行之间移动，与筛选前一致
