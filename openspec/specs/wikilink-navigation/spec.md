# wikilink-navigation Specification

## Purpose

定义 wikilink 的显示与激活口径：md 模式装饰层在视口内定位链接 span，并按 Rust 解析结果三态显示（正常 / 歧义 / 未创建）；激活链接打开目标文件并定位标题行，锚点缺失时提示「标题未找到」；未创建链接提供显式的一键创建入口。键位走统一键位表、chorded 且非 modal；解析语义一律取自 Rust，webview 层不复制。由 change `add-wikilink` 归档并入（2026-09-17）。
