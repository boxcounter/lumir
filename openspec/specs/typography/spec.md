...
定义编辑器排版的落地口径：编辑器作用域的排版 token 层（正文族 `--editor-font-family`、等宽族
`--editor-mono-family`、内容字号 `--editor-font-size`）与其配置来源（`[editor]` 的 `font_family` /
`mono_font_family` / `font_size` 三个字段，含合法区间 `[12, 32]` 与非法值回落），运行期字号步进的
三条命令（倍率 1.1 取整、钳制、重置回配置值，且不写文档、不落盘），排版变更后的重测量，以及
「出厂默认观感与本 change 之前逐项相同」这条硬约束。配置只影响编辑器，shell（左栏、masthead、浮层、
键位面板、搜索面板）不受影响——由 token 分层结构性保证，MUST NOT 依赖逐处记得改。由 change
`typography-and-zoom` 归档并入（2026-09-24，实现 M195）。
...
- **THEN** 既有整页基线（本 change 落地时 30 张）逐张零差异；判据 SHALL 是内容级像素对比
  （全量比对通过 + 基线文件零改写），MUST NOT 只看时间戳；本 change 新增的基线只增不改
