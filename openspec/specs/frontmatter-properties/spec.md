...
定义 md 文件 YAML frontmatter 的 properties 区块渲染（ADR 0003 §1 兼容范围）：键值表格形态替换源码显示、tags 标签形态、非法 YAML 回退原文不丢内容；跨行 replace 装饰受 CM6 硬限制走 StateField 构建。由 change `add-editor-live-preview` 归档并入（2026-09-05，实现 M19；归档时补写构建策略约束，见归档 tasks.md 第 6 节透明声明）。
