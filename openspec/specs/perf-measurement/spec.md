# perf-measurement Specification
...
- **THEN** 输出附带下界声明（三条：无 OS 输入管道/合成器开销、Blink 与 WebKit 引擎差异、未覆盖 Foundation 真实可编辑文档更新与绘制路径）

#### Scenario: Foundation 真实编辑端点独立演进

- **WHEN** Foundation 可编辑路径具备端到端测量条件
- **THEN** 后续 OpenSpec change 新增覆盖真实 WebKit 文档更新与绘制路径的独立端点，并保留现有 Blink 端点作为结构下界

### Requirement: 打开 1MB Markdown 文件测量

打开 1MB 文件的现有端点 SHALL 保持占位口径：从 `fs.readFile` 开始到文件内容完成 UTF-8 解码，纯磁盘 IO + 解码，不含 Rust core 文件读取、Markdown 解析、CodeMirror 装载或首帧渲染；N=50、page cache 热，细节见 docs/specs/perf-measurement.md 第 3 节。该读数只是 IO/解码结构下界，MUST NOT 单独引用为 Foundation 真实首屏打开性能。fixture SHALL 为恰好 1,048,576 字节（1 MiB）的确定性 Markdown 文件并提交入库，内容 MUST 只能通过确定性再生成器变更，以保证历史读数可比较。Foundation 真实首屏性能 SHALL 通过后续 OpenSpec change 新增独立演进端点「打开请求发出 → 文档在编辑器完成首帧渲染」；现有端点继续保留为 IO/解码下界，不以未来端点替换。
...
#### Scenario: Foundation 真实首屏端点独立演进

- **WHEN** Foundation 真实打开路径具备端到端测量条件
- **THEN** 后续 OpenSpec change 新增含 Rust core 读取、解析、CodeMirror 装载与首帧渲染的独立端点，并保留现有 open-file 端点作为 IO/解码结构下界
