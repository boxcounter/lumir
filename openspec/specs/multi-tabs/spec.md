...
定义多标签会话的落地口径：单一 `EditorView` + 每标签独立 `EditorState`（切换经 `view.setState`，撤销史/语法树/选区随标签留存），标签栏的显示与形态、打开与固定语义（预览标签 vs 固定标签）、按标签隔离的 dirty 判定与保存粒度、文件树高亮联动与空态。由 change `add-multi-tabs` 归档并入（2026-09-17，实现 M149），并由 change `remember-reading-position` 归档并入（2026-09-24，实现 M194——打开文档时按 vault 恢复上次阅读位置）。
