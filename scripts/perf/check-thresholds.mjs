...
//    浮动项（spreadHeadroom）只松不紧，且只在窗口自身已含高端读数时生效。按同一穷举法，k≥minRuns=5 的
//    窗口高侧散布上界是 57.9%（窗口 22.35/26.75/33.70/33.90/53.20，median 33.70）×1.2 = 69.4%，**高于**
//    60% 配置值——即该窗口下浮动项会把容忍线抬到配置值以上；而完整 9 条窗口是 33.7%×1.2=40.4%、S3b 那个
//    低端紧致角落窗口是 18.1%×1.2=21.7%，两者低于 60%、浮动项惰性。护栏方向是「窗口抖动大→容忍线跟着放宽，
//    runner 稳定→回落配置值」，不会像静态常数那样在两种 runner 状态里必错一边。
