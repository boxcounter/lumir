...
steps:
  ...
  - name: 拖右缘手柄向右 80pt：松手后栏宽 840（680+2×80 对称律）写回 config.json
    do: drag
    target: { textareaEdge: "right" }
    dx: 80
    expect:
      - label: config.json 出现了 ui.content_width = 840（D3：松手写一次，通用键值合并写通道；位移确定值，钉字面量）
        file: { path: "env:config.json", has: '"content_width": 840' }
  ...
  - name: 拖左缘手柄向左 40pt（左缘向左 = 放宽；对称手柄同效的另一侧）：840 + 2×40 = 920
    do: drag
    target: { textareaEdge: "left" }
    dx: -40
    expect:
      - label: content_width = 920（合并写通道第二次落盘，左缘手柄对称生效）
  ...
  - name: 重启后栏宽保持（D3 的持久化语义：config.json 的值被启动读取）
    do: restart
    expect:
      - label: 重启后 config.json 的 content_width = 920 仍在（持久化不是运行期假象）
        file: { path: "env:config.json", has: '"content_width": 920' }
  ...
  - name: 重开 keys.md：栏宽配置下编辑器照常装载
    ...
    file: keys.md
    expect:
      - label: 编辑器照常装载（920 的配置值经启动读取 → 生效，不报错不白屏）
        editor: { has: "键位场景" }
      - shot: 重启后-栏宽保持

  - name: 下限钳制：从当前值大幅往窄拖（位移足以越过钳制阈值 120pt），宽度停在 680（默认值即下限，D2）
    do: drag
    target: { textareaEdge: "right" }
    dx: -400
    expect:
      - label: 钳到下限 680（越界回落端点，不是负值也不是报错）
        file: { path: "env:config.json", has: '"content_width": 680' }
teardown:
...
- dx 是**窗口局部点**（CGEvent 通道与 doubleClick 同口径，不经过截图像素的 0.96 缩放），
  因此栏宽增量是确定值（±80pt → 宽度 ±160），断言钉字面量 840 / 920 / 680。
