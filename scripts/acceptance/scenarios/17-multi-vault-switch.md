...
2. **两个固定标签与按 vault 的会话**：固定标签经「单击打开 → 键入（首次输入即固定，不持久化预览
   标签）」这条真实用户路径得到——**不能用双击**：双击走的是 AX 索引点击（AXPress ×2），产生的
   是两个独立 click，前端的 `dblclick` 收不到（套件也没有 `target.count` 的 x/y 变体），
   因此「双击固定」这条路径由 chromium 视觉通道覆盖（`tests/visual/scenes/m149-tabs.spec.ts`）。
