...
function extensions() {
  ...
  return [EditorState.readOnly.of(true), EditorView.domEventHandlers({focusin(event, editor) { const target = event.target; if (!(target instanceof HTMLElement) || !target.matches('.table-scroll') || new URLSearchParams(location.search).has('fault-focus')) return false; const selection = editor.dom.ownerDocument.getSelection(); if (!selection?.anchorNode || !editor.contentDOM.contains(selection.anchorNode)) { editor.focus(); const anchor = editor.domAtPos(editor.state.selection.main.anchor); const head = editor.domAtPos(editor.state.selection.main.head); selection?.setBaseAndExtent(anchor.node, anchor.offset, head.node, head.offset); target.focus({preventScroll: true}); } return false; }, wheel(event) { const target = event.target instanceof Element ? event.target.closest<HTMLElement>('.table-scroll') : null; if (!target || !event.deltaX) return false; target.scrollLeft += event.deltaX; event.preventDefault(); return true; }, keydown(event) { const target = event.target; if (!(target instanceof HTMLElement) || !target.matches('.table-scroll')) return false; if (['ArrowLeft','ArrowRight','Home','End'].includes(event.key)) { target.scrollLeft = event.key === 'Home' ? 0 : event.key === 'End' ? target.scrollWidth : target.scrollLeft + (event.key === 'ArrowRight' ? 120 : -120); event.preventDefault(); return true; } if (event.key === 'Escape') { view.focus(); event.preventDefault(); return true; } return false; }}), EditorView.lineWrapping, EditorView.blockWrappers.of(BlockWrapper.set(wrappers, true)), plugin, EditorView.updateListener.of(() => { status.textContent = `doc unchanged=${view?.state.doc.toString() === source}; selection=${view?.state.selection.main.from}..${view?.state.selection.main.to}`; })];
}
