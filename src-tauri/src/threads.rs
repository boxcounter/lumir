...
pub fn vault_id(path:&str)->String { let mut h:u64=1469598103934665603; for b in path.as_bytes(){h^=*b as u64; h=h.wrapping_mul(1099511628211)} format!("vault-{h:016x}") }
pub fn reconcile_vault(path:&std::path::Path)->Result<(),CommandError>{let p=path.display().to_string(); let id=vault_id(&p); let _=vault_register(id,p); Ok(())}
